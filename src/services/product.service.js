...
} = require("../models/reposistories/inventory.repo");
const {
  findAllDraftProduct,
...
class ProductFactory {
  ...
  static async getPublishedProductInShop({
    productShop,
    skip = 0,
    limit = 20,
  }) {
  ...
  static async getAllProductsInShop({
    limit = 50,
    sort = "ctime",
    page = 1,
...
class Product {
  ...
  async createProduct(productId) {
    ...
    if (newProduct) {
      await insertProductToInventory({
        productId: newProduct._id,
        productShop: newProduct.productShop,
...
class Perfume extends Product {
  async createProduct() {
    ...
    if (!newPerfume)
      throw new BadRequestErrorResponse("create new perfume error");

    const newProduct = await super.createProduct(newPerfume._id);
    if (!newProduct)
      throw new BadRequestErrorResponse("create new Product error");
  ...
  async updateProductClass(productId) {
    ...
    if (objectParams.productAttribute) {
      await updateProductById({
        model: perfume,
        productId,
        payload: updateNestedObjects(objectParams.productAttribute),
      });
    }

    return await super.updateProduct(
      productId,
      updateNestedObjects(objectParams)
    );
  }
}
...
class Cosmetic extends Product {
  async createProduct() {
    ...
    if (!newCosmetic)
      throw new BadRequestErrorResponse("create new cosmetic error");
